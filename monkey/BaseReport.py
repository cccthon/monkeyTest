...
# from monkey import BaseAnalysis,  BaseCashEmnu as go
from monkey import BaseAnalysis
from monkey.BasePickle import readInfo
...
class OperateReport:
        ...
        for t in info:
            for wrap in t:
                for item in t[wrap]:
                    ...
                    _write_center(worksheet, "B" + str(temp), str(t[wrap]["header"]["kel"])+"核", self.wd)
                    _write_center(worksheet, "C" + str(temp), str(math.ceil(int(t[wrap]["header"]["rom"]) / 1024)) + "M", self.wd)
                    ...
                    _write_center(worksheet, "J" + str(temp), BaseAnalysis.avgMen(men, int(t[wrap]["header"]["rom"])), self.wd)
    ...
    def getCrashMsg(self, log):
        ...
        print(log)
        print(log.split("_")[2])
        devices = log.split("_")[2]
        with open(log, encoding="utf-8") as monkey_log:
            lines = monkey_log.readlines()
            for line in lines:
                if re.findall("ANR", line):
                    print("存在anr错误: " + devices + "  " + line)
                    self._crashM.append(devices + "  " + line.strip("\n"))
                if re.findall("CRASH", line):
                    print("存在crash错误: " + devices + "  " + line)
                    self._crashM.append(devices + "  " + line.strip("\n"))
                if re.findall("Exception", line):
                    print("存在crash错误: " + devices + "  " + line)
                    self._crashM.append(devices + "  " + line.strip("\n"))
        print()
        print("crashhhhhhhhhhhhhh: ", self._crashM)
        print()
        writeInfo(self._crashM, PATH(f"../info/{devices}_monkeyLog.pickle"))
...
if __name__ == '__main__':
    ...
    info = [{'12345': {'cpu': '/Users/xxxxxxxxx/workspace/github/airTest_monkey/info/192.168.20.78:5555_cpu.pickle', 
    'battery': '/Users/xxxxxxxxx/workspace/github/airTest_monkey/info/192.168.20.29:5556_battery.pickle', 
    'men': '/Users/xxxxxxxxx/workspace/github/airTest_monkey/info/192.168.20.78:5555_men.pickle', 
    'flow': '/Users/xxxxxxxxx/workspace/github/airTest_monkey/info/192.168.20.29:5556_flow.pickle', 
    'header': {'rom': 770300, 'kel': '2核', 'monkey_log': 
    '/Users/xxxxxxxxx/workspace/github/airTest_monkey/log/20200310115902_192.168.20.29:5556_monkey.log', 'beforeBattery': 99, 
    'pix': '1440x810', 'time': '10秒', 'afterBattery': 99, 'phone_name': 'GT-I9500_samsung_4.4', 'net': 'gprs'}, 
    'fps': '/Users/xxxxxxxxx/workspace/github/airTest_monkey/info/192.168.20.78:5555_fps.pickle'}}]
